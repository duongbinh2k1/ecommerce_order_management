...
class TestInventoryLogEntry(unittest.TestCase):
    ...
    def test_invalid_quantity_change_validation(self) -> None:
        ...
        invalid_quantities = ["10", 1.5, None, []]
        
        for invalid_qty in invalid_quantities:
            with self.subTest(quantity_change=invalid_qty):
                with self.assertRaises(ValueError) as context:
                    InventoryLogEntry(
                        product_id=self.valid_product_id,
                        quantity_change=invalid_qty,  # type: ignore
                        reason=self.valid_reason
                    )
                self.assertIn("Quantity change must be an integer", str(context.exception))
    ...
    def test_invalid_reason_validation(self) -> None:
        ...
        invalid_reasons: list[Any] = ["", "   ", None, 123, []]
        ...
        for invalid_reason in invalid_reasons:
            with self.subTest(reason=invalid_reason):
                ...
                self.assertIn("Reason must be a non-empty string", str(context.exception))
    ...
    def test_immutability(self) -> None:
        ...
        with self.assertRaises(AttributeError):
            entry.product_id = 2000  # type: ignore
        with self.assertRaises(AttributeError):
            entry.quantity_change = -100  # type: ignore
        with self.assertRaises(AttributeError):
            entry.reason = "sale"  # type: ignore
        with self.assertRaises(AttributeError):
            entry.timestamp = datetime.datetime.now()  # type: ignore
